...
// sys_write(buf, sz)
//    Write data to kernel from `buf`. Writes at most `sz` bytes.
//    Returns number of bytes written or -1 on error.
__noinline ssize_t sys_write(const void* buf, size_t sz) {
    return make_syscall(SYSCALL_WRITE, (uintptr_t) buf, sz);
}

// sys_read(buf, sz)
//    Read data from kernel into `buf`. Reads at most `sz` bytes.
//    Returns number of bytes read or -1 on error.
__noinline ssize_t sys_read(void* buf, size_t sz) {
    return make_syscall(SYSCALL_READ, (uintptr_t) buf, sz);
}
