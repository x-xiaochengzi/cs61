...
void kernel_start(const char* command) {
    ...
    if (!program_image(command).empty()) {
        ...
        process_setup(1, "writer");
        process_setup(2, "reader");
    } else {
...
ssize_t syscall_write(const char* buf, size_t sz);
ssize_t syscall_read(char* buf, size_t sz);
...
// syscall_write(buf, sz)
//    Handles the SYSCALL_WRITE system call; see `sys_write` in `u-lib.hh`.
...
// syscall_read(buf, sz)
//    Handles the SYSCALL_READ system call; see `sys_read` in `u-lib.hh`.
